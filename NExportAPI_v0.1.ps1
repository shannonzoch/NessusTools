...
.DESCRIPTION
    Connects to Nessus API, downloads scans, and generates HTML/PDF reports with filtering by plugin publication date.
...
.EXAMPLE
    .\Generate-NessusAPIReport.ps1 -NessusUrl "https://nessus.local:8834" -AccessKey "abc123" -SecretKey "xyz789" -FolderName "Production" -PluginDate "2024-01-01"
    
.EXAMPLE
    .\Generate-NessusAPIReport.ps1 -NessusUrl "https://nessus.local:8834" -AccessKey "abc123" -SecretKey "xyz789" -ListFolders
    
.EXAMPLE
    .\Generate-NessusAPIReport.ps1 -NessusUrl "https://nessus.local:8834" -AccessKey "abc123" -SecretKey "xyz789" -ScanId 123 -OutputPrefix "weekly_scan"
#>
...
try {
    ...
    Write-Host "=== Nessus API Report Generator ===" -ForegroundColor Cyan
    Write-Host ""
    ...
    foreach ($scan in $scansToExport) {
        $scanId = $scan.id
        Write-Host "`n[+] Processing scan ID: $scanId" -ForegroundColor Green
